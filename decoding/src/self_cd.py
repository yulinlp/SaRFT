...
class SelfCDDecoding:
    ...
    def generate(self, inputs, alpha=2.5, max_token_len=256, gen_config=None, **kwargs):
        ...
        while step <= max_token_len:  # Loop until we reach the first m tokens
            ...
            rose_scores = output_safe.scores - alpha * (
                output_safe.scores - output_unsafe.scores
            )
            ...
            # print(
            #     f"step: {step}\nrose_token_id: {rose_token_id}\norigin_token_id: {selected_token_id}"
            # )
