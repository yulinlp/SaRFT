...
class SafeDecoding:
    ...
    def generate(self, inputs, gen_config=None, **kwargs):
        ...
        while step <= min(
            max_token_len, self.first_m
        ):  # Loop until we reach the first m tokens
            ...
            while len(common_tokens) < self.num_common_tokens:
                ...
                common_in_iteration = set(current_indices_base.tolist()) & set(
                    current_indices_expert.tolist()
                )
                ...
                if iter_range > min(
                    len(sorted_indices_base), len(sorted_indices_expert)
                ):
                    break
            ...
            if self.verbose and step == 1:
                ...
                logging.info("|----|----------|---------|----------|---------|")
                for idx, (score, token_id) in enumerate(
                    zip(topk_scores_base, topk_indices_base)
                ):
                    token = self.tokenizer.decode(token_id.item())
                    prob = torch.exp(score)
                    logging.info(
                        f"{idx+1:4d} | {token_id:8d} | {token:7s} | {score:.3f}    | {prob:.2%} |"
                    )
                ...
                logging.info("|----|----------|---------|----------|---------|")
                for idx, (score, token_id) in enumerate(
                    zip(topk_scores_expert, topk_indices_expert)
                ):
                    token = self.tokenizer.decode(token_id.item())
                    prob = torch.exp(score)
                    logging.info(
            ...
            intersection_indices = torch.tensor(
                list(common_tokens), device=self.model.device
            )
            ...
            for token_id in intersection_indices:
                ...
                # Steer probabilities
                prob_diff = torch.exp(scores_expert[token_id]) - torch.exp(
                ...
                updated_prob = (
                    updated_prob
                    if updated_prob > 0
                    else torch.tensor(1e-8, device=self.model.device)
                )
            ...
            normalized_probs = torch.nn.functional.softmax(
                torch.tensor(updated_scores).float(), dim=0
            )

            sorted_indices = sorted(
                range(len(normalized_probs)),
                key=lambda i: normalized_probs[i],
                reverse=True,
            )
            ...
            if self.verbose:
                ...
                logging.info("|No. | Token ID | Token   | Log Prob | Prob    |")
                logging.info("|----|----------|---------|----------|---------|")
                for idx, (prob, token_id) in enumerate(
                    zip(sorted_probs, sorted_token_ids)
                ):
                    token = self.tokenizer.decode(token_id.item())
            ...
            if do_sample == False:
                ...
                selected_token_id = sorted_top_p_token_ids[
                    torch.multinomial(torch.softmax(sorted_top_p_scores, dim=-1), 1)
                ].unsqueeze(0)
            else:
            ...
            inputs["input_ids"] = torch.cat(
                [inputs["input_ids"], selected_token_id.unsqueeze(0)], dim=1
            )
            inputs["attention_mask"] = torch.cat(
                [
                    inputs["attention_mask"],
                    torch.tensor([[1]], device=self.model.device),
                ],
                dim=1,
            )
